...
class TwoDimensionalCustomPaintGridView extends StatelessWidget {
  ...
  void onTapDown(BuildContext context, TapDownDetails details) {
    ...
    final itemWidth = data[0].isNotEmpty
        ? context.size!.width / data[0].length
        : 0.0; // Handle potential division by zero
    final itemHeight =
        data.isNotEmpty ? context.size!.height / data.length : 0.0;
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final (paintWidth, paintHeight) = (
          gameState.data.first.length * itemSize,
          gameState.data.length * itemSize
        );
            ...
            child: ClipRRect(
              child: GestureDetector(
                onTapDown: (tapDownDetails) =>
                    onTapDown(context, tapDownDetails),
                child: RepaintBoundary(
                  child: CustomPaint(
                    ...
                    foregroundPainter:
                        GuideLinePainter(separator: 5, itemSize: itemSize),
                    painter: GOFPainter(
